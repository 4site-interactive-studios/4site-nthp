...
export class VGS {
  ...
  constructor() {
    ...
    this._form.onValidate.subscribe(() => {
      if (this._form.validate) {
        ...
        VGS.log(`Form Validation: ${isValid}`);
        this._form.validate = isValid;
      }
    });
  }

  static log(message: string, color: string = "#FFF", bg: string = "#000") {
  ...
  setDefaults() {
    ...
    this.options = CommonEN.deepMerge(defaultOptions, options);
  }
  setPaymentType() {
    // If there's no default payment type, set the default to card
    if (CommonEN.getPaymentType() === "") {
      CommonEN.setPaymentType("card");
    }
  }
  dumpGlobalVar() {
  ...
  private handleExpUpdate = (e: string) => {
    ...
    if (e == "month") {
      ...
      VGS.log(`month disable ${disable}`);
      VGS.log(`selected_month ${selected_month}`);
      ...
      VGS.log(`year disable ${disable}`);
      VGS.log(`selected_year ${selected_year}`);
  ...
  private validate() {
    if (
      ...
      if (cardContainer && cardEmpty) {
        window.setTimeout(() => {
          CommonEN.setError(cardContainer, "Please enter a valid card number");
          // Scroll to the error
      ...
      if (cvvContainer && cvvEmpty) {
        window.setTimeout(() => {
          CommonEN.setError(cvvContainer, "Please enter a valid CVV");
          // Scroll to the error
